...
(define rig_even
  (lambda (riga even k i) ; riga, valore even, selezione riga, selezione even
    (if (and (not (= 1 (/ even (sel riga k))))
             (= 0 (remainder even (sel riga k))))
        (/ even (sel riga k))
        (if (= k (length riga))
            (rig_even riga (sel riga (+ i 1)) 1 (+ i 1))
            (rig_even riga even (+ k 1) i)))
    ))

(define even
  (lambda (riga) ;riga, valore even, k
    (rig_even riga (sel riga 1) 1 1)
    ))
